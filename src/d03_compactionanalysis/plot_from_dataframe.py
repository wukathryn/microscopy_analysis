...
def plot_timepoints(df, graphs_dir, ydata_labels, splitby, display):
    ...
    for val in splitby_vals:
        ...
        for i, ylabel in enumerate(ydata_labels):
            ...
            sns.lineplot(data=df_selection, x=xdata_label, y=ylabel, hue='unique cell index', ax=axs_r[i], legend=False)
...
def plot_summary_metrics(df, graphs_dir, xdata_label, display):
    ...
    grouped_df = df.groupby(['unique cell index']).apply(df_groupby_calculations).reset_index()
...
def df_groupby_calculations(x):
    d = {}
    d['day'] = x['day'].iloc[0]
    d['max timepoint'] = x['timepoint'].max()
...
def plot_from_dataframe(df, graphs_dir, display=False):
    ...
    #plot_timepoints(df, graphs_dir, ydata_labels, splitby='day', display=display)
    #plot_timepoints(df, graphs_dir, ydata_labels, splitby='unique cell index', display=display)
    plot_summary_metrics(df, graphs_dir, xdata_label='day', display=display)
...
def main():
    ...
    df = pd.read_csv(os.path.join(graphs_dir, df_name))
